...
- `omk setup --scope user|project`를 통한 **범위 인식 설정** — 유연한 설치 모드.
...
omk setup          # 범위별 프롬프트/스킬/설정 설치 + 프로젝트 AGENTS.md/.omk
...
OMK는 이제 플러그인 스캐폴딩 및 검증을 위한 `omk hooks`를 포함합니다.
...
팀이 ralph 모드(`omk team ralph ...`)로 실행될 때, 종료 정리는
...
`omk team shutdown <name> --ralph`로 명시적으로 전달할 수 있습니다.

팀 워커를 위한 Worker CLI 선택:

```bash
OMK_TEAM_WORKER_CLI=auto    # 기본값; worker --model에 "claude"가 포함되면 claude 사용
OMK_TEAM_WORKER_CLI=codex   # Codex CLI 워커 강제
...
- Claude worker 모드에서 OMK는 워커를 일반 `claude`로 시작하고 (추가 시작 인수 없음) 명시적인 `--model` / `--config` / `--effort` 재정의를 무시하여 Claude가 기본 `settings.json`을 사용합니다.
...
- `config.toml` 업데이트 (두 범위 모두):
  ...
  - MCP 서버 항목 (`omk_state`, `omk_memory`, `omk_code_intel`, `omk_trace`)
  - `[tui] status_line`
- 프로젝트 `AGENTS.md`
...
- **[전체 문서](https://penghuo.github.io/oh-my-kiro-website/docs.html)** — 완전한 가이드
- **[CLI 레퍼런스](https://penghuo.github.io/oh-my-kiro-website/docs.html#cli-reference)** — 모든 `omk` 명령어, 플래그 및 도구
- **[알림 가이드](https://penghuo.github.io/oh-my-kiro-website/docs.html#notifications)** — Discord, Telegram, Slack 및 webhook 설정
- **[권장 워크플로우](https://penghuo.github.io/oh-my-kiro-website/docs.html#workflows)** — 일반적인 작업을 위한 실전 검증된 스킬 체인
- **[릴리스 노트](https://penghuo.github.io/oh-my-kiro-website/docs.html#release-notes)** — 각 버전의 새로운 기능
