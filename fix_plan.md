...
- [x] Update package.json metadata: set description to "Multi-agent orchestration layer for Kiro CLI", verify bin field is {"omk":"bin/omk.js"}, verify GitHub URLs point to penghuo/oh-my-kiro, replace keywords codex/openai with kiro/kiro-cli, set author to "Peng Huo". Commit.
- [x] Update .gitignore: add .omk/ and .kiro/ entries, keep all existing entries (.omc/, .omx/, .codex/, .claude/, .agents/). Commit.
- [x] Regenerate package-lock.json: rm package-lock.json, npm install. Commit.
