...
| `$learn-about-omk` / `$learner` / `$writer-memory` | No in-repo replacement | Remove stale references from workflows/docs. |
...
- [ ] Confirm removed references are gone from local notes/scripts:
  ```bash
  rg -n "deep-executor|scientist|pipeline|project-session-manager|\bpsm\b|ultrapilot|learn-about-omk|writer-memory|learner|deepinit|\brelease\b" README.md docs scripts .omk -S
  ```
