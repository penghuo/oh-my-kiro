...
// ---------------------------------------------------------------------------
// Kiro worker agent config generation
// ---------------------------------------------------------------------------

async function generateKiroWorkerAgentConfig(
  teamName: string,
  workerName: string,
  workerIndex: number,
...
export async function startTeam(
  ...
  try {
    ...
    // 5b. Generate Kiro agent configs for kiro workers (before tmux session creation)
    for (const plan of workerBootstrapPlans) {
      if (plan.workerCli === 'kiro') {
        await generateKiroWorkerAgentConfig(
          sanitized,
          plan.workerName,
          workerBootstrapPlans.indexOf(plan) + 1,
          plan.instructionsFilePath,
          leaderCwd,
          teamStateRoot,
          plan.workerRole,
        );
...
export async function shutdownTeam(teamName: string, cwd: string, options: ShutdownOptions = {}): Promise<void> {
  ...
  // 5b. Cleanup generated Kiro agent configs
  try {
    await cleanupKiroWorkerAgents(cwd, config.workers.length);
  } catch (err) {
    process.stderr.write(`[team/runtime] kiro agent cleanup failed: ${err}\n`);
  }
