  ...
  return await withScalingLock(sanitized, leaderCwd, async (): Promise<ScaleUpResult | ScaleError> => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (uniqueTaskRoles.size > 1) {
        console.log(`[omk:scaling] ${workerName}: mixed task roles [${[...uniqueTaskRoles].join(', ')}], falling back to ${agentType}`);
      }
      ...
      if (!skipReadyWait) {
        ...
        if (!ready) {
          console.log(`[omk:scaling] Warning: worker ${workerName} did not become ready within timeout`);
        }
      }
