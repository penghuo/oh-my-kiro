...
export type TeamWorkerCli = 'codex' | 'claude' | 'gemini' | 'kiro';
...
function isHudWatchPane(pane: TmuxPaneInfo): boolean {
  ...
  return /\bomk\b.*\bhud\b.*--watch/i.test(start);
}
...
function normalizeTeamWorkerCliMode(raw: string | undefined, sourceEnv: string = OMK_TEAM_WORKER_CLI_ENV): TeamWorkerCliMode {
  ...
  if (normalized === 'codex' || normalized === 'claude' || normalized === 'gemini' || normalized === 'kiro') return normalized;
  throw new Error(`Invalid ${sourceEnv} value "${raw}". Expected: auto, codex, claude, gemini, kiro`);
}
...
export function translateWorkerLaunchArgsForCli(workerCli: TeamWorkerCli, args: string[], initialPrompt?: string): string[] {
  ...
  if (workerCli === 'kiro') {
    // Kiro uses agent configs for prompt/MCP/hooks — no Codex-style -c flags needed.
    // The agent name is injected by buildWorkerProcessLaunchSpec via --agent flag.
    // --trust-all-tools replaces --dangerously-bypass-approvals-and-sandbox.
    const model = extractModelOverride(args);
    const translated: string[] = ['chat', '--trust-all-tools'];
    if (model) translated.push(MODEL_FLAG, model);
    return translated;
  }
  if (workerCli === 'gemini') {
    const model = extractModelOverride(args);
...
export function assertTeamWorkerCliBinaryAvailable(
  ...
  const binary = workerCli === 'kiro' ? 'kiro-cli' : workerCli;
  if (existsImpl(binary)) return;
  throw new Error(
    `Selected team worker CLI "${workerCli}" is not available on PATH. `
      + `Install "${binary}" or set ${OMK_TEAM_WORKER_CLI_ENV}=codex|claude|gemini|kiro.`,
  );
}
...
export function buildWorkerProcessLaunchSpec(
  ...
  let effectiveCliLaunchArgs = workerCli === 'codex' && !cliLaunchArgs.includes(CODEX_BYPASS_FLAG)
    ? [...cliLaunchArgs, CODEX_BYPASS_FLAG]
    : cliLaunchArgs;

  // Kiro workers use --agent flag pointing to a dynamically generated agent config.
  // The agent name follows the convention omk-worker-{index}.
  if (workerCli === 'kiro') {
    const agentName = `omk-worker-${workerIndex}`;
    effectiveCliLaunchArgs = [...cliLaunchArgs, '--agent', agentName];
  }

  const cliBinary = workerCli === 'kiro' ? 'kiro-cli' : workerCli;
  const resolvedCliPath = resolveAbsoluteBinaryPath(cliBinary);
 ...
 * OMK requires tmux, which is unavailable on native Windows.
 */
...
export function createTeamSession(
  ...
  try {
    ...
    const omkEntry = process.argv[1];
    if (omkEntry && omkEntry.trim() !== '') {
      const hudCmd = `node ${shellQuoteSingle(translatePathForMsys(omkEntry))} hud --watch`;
...
export function restoreStandaloneHudPane(
  ...
  const omkEntry = process.argv[1];
  if (!omkEntry || omkEntry.trim() === '') return null;
