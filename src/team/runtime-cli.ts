 ...
 *
 * Spawned by omk_run_team_start in state-server.ts.
 */
...
type TeamWorkerProvider = 'codex' | 'claude' | 'gemini' | 'kiro';
...
async function writePanesFile(
  ...
  const omkJobsDir = process.env.OMK_JOBS_DIR;
  if (!jobId || !omkJobsDir) return;

  const panesPath = join(omkJobsDir, `${jobId}-panes.json`);
  await writeFile(
    panesPath + '.tmp',
...
export function normalizeAgentTypes(raw: string[], workerCount: number): TeamWorkerProvider[] {
  ...
  const invalid = providers.filter((entry) => entry !== 'codex' && entry !== 'claude' && entry !== 'gemini' && entry !== 'kiro');
  if (invalid.length > 0) {
    throw new Error(`Invalid agentTypes entries: ${invalid.join(', ')}. Expected codex|claude|gemini|kiro.`);
  }
...
async function main(): Promise<void> {
  ...
  // Start the team — OMK's startTeam takes individual parameters
  ...
  // Persist pane IDs so MCP server can clean up explicitly via omk_run_team_cleanup.
