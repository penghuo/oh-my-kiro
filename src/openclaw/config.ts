  ...
  if (cliEnabled && cliAlias) {
    ...
    applyHooks(
      ...
      parseInstruction(
        ...
        "OMK event {{event}} for {{projectPath}}",
      ),
      "custom_cli_command",
    );
  }
...
export function getOpenClawConfig(): OpenClawConfig | null {
  ...
  try {
    ...
    const omkConfigPath = join(codexHome(), ".omk-config.json");
    if (!existsSync(omkConfigPath)) {
      _cachedConfig = undefined;
      return null;
    }

    const fullConfig = JSON.parse(readFileSync(omkConfigPath, "utf-8")) as Record<string, unknown>;
