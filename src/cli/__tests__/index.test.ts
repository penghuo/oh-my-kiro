...
describe('resolveNotifyTempContract', () => {
  ...
  it('activates from OMK_NOTIFY_TEMP=1 env parity', () => {
    const parsed = resolveNotifyTempContract(['--model', 'gpt-5'], { OMK_NOTIFY_TEMP: '1' });
    assert.equal(parsed.contract.active, true);
...
describe('project launch scope helpers', () => {
  ...
  it('migrates legacy "project-local" persisted scope to "project"', async () => {
    ...
    try {
      await mkdir(join(wd, '.omk'), { recursive: true });
      await writeFile(join(wd, '.omk', 'setup-scope.json'), JSON.stringify({ scope: 'project-local' }));
      assert.equal(readPersistedSetupScope(wd), 'project');
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
...
describe('tmux HUD pane helpers', () => {
  it('findHudWatchPaneIds detects stale HUD watch panes and excludes current pane', () => {
    const panes = parseTmuxPaneSnapshot(
      [
        '%1\tzsh\tzsh',
        '%2\tnode\tnode /tmp/bin/omk.js hud --watch',
        '%3\tnode\tnode /tmp/bin/omk.js hud --watch',
...
describe('detached tmux new-session sequencing', () => {
  it('buildDetachedSessionBootstrapSteps uses shared HUD height and split-capture ordering', () => {
    ...
    assert.equal(steps[0]?.args.includes('OMK_NOTIFY_TEMP_CONTRACT={\"active\":true}'), true);
  });
  ...
  it('buildDetachedSessionBootstrapSteps forwards temp contract env to detached tmux session', () => {
    ...
    assert.equal(
      ...
      && newSession!.args.some((arg) => arg.startsWith('OMK_NOTIFY_TEMP_CONTRACT=')),
      true,
    );
  });
  ...
  it('buildDetachedSessionFinalizeSteps keeps schedule after split-capture and before attach', () => {
    const steps = buildDetachedSessionFinalizeSteps('omk-demo', '%12', '3', true, false);
  ...
  it('buildDetachedSessionFinalizeSteps uses quiet best-effort tmux resize commands', () => {
    const steps = buildDetachedSessionFinalizeSteps('omk-demo', '%12', '3', false, false);
  ...
  it('buildDetachedSessionFinalizeSteps skips detached resize hooks on native Windows', () => {
    const steps = buildDetachedSessionFinalizeSteps('omk-demo', '%12', '3', true, false, true);
    assert.deepEqual(
  ...
  it('buildDetachedSessionRollbackSteps unregisters hooks before killing session', () => {
    const steps = buildDetachedSessionRollbackSteps(
      'omk-demo',
      'omk-demo:0',
      'omk_resize_launch_demo_0_12',
      'omk_attached_launch_demo_0_12',
    );
    assert.deepEqual(
      steps.map((step) => step.name),
  ...
  it('buildDetachedSessionRollbackSteps only kills session when no hook metadata exists', () => {
    const steps = buildDetachedSessionRollbackSteps('omk-demo', null, null, null);
...
describe('buildTmuxSessionName', () => {
  it('uses detached fallback quietly outside git repos', () => {
    const name = buildTmuxSessionName('/tmp/My Repo', 'omk-1770992424158-abc123');
    assert.equal(name, 'omk-my-repo-detached-1770992424158-abc123');
  });

  it('sanitizes invalid characters', () => {
    const name = buildTmuxSessionName('/tmp/@#$', 'omk-+++');
    assert.match(name, /^omk-(unknown|[a-z0-9-]+)-[a-z0-9-]+-(unknown|[a-z0-9-]+)$/);
    assert.equal(name.includes('_'), false);
    assert.equal(name.includes(' '), false);
  });

  it('includes repo name when cwd is inside .omk-worktrees', () => {
    const name = buildTmuxSessionName('/home/user/my-repo.omk-worktrees/launch-feature-x', 'omk-123-abc');
    assert.match(name, /^omk-my-repo-launch-feature-x-/);
  });

  it('includes repo name for detached worktree paths', () => {
    const name = buildTmuxSessionName('/projects/cool-project.omk-worktrees/launch-detached', 'omk-456-def');
...
describe('injectModelInstructionsBypassArgs', () => {
  ...
  it('uses session-scoped default model_instructions_file when provided', () => {
    ...
    assert.deepEqual(
      args,
      ['--model', 'gpt-5', '-c', 'model_instructions_file="/tmp/my-project/.omk/state/sessions/session-1/AGENTS.md"']
    );
  });
});
