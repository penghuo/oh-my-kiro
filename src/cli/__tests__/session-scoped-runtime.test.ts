...
function runOmk(cwd: string, ...args: string[]) {
  return spawnSync(process.execPath, [omkBin, ...args], {
    cwd,
    encoding: 'utf-8',
  });
}
...
describe('CLI session-scoped state parity', () => {
  it('status and cancel include session-scoped states', async () => {
    ...
    try {
      ...
      const statusResult = runOmk(wd, 'status');
      ...
      const cancelResult = runOmk(wd, 'cancel');
