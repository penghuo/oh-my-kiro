...
export const AGENTS_INIT_USAGE = [
  'Usage: omk agents-init [path] [--dry-run] [--force] [--verbose]',
  '       omk deepinit [path] [--dry-run] [--force] [--verbose]',
  '',
...
const MANAGED_MARKER = '<!-- OMK:AGENTS-INIT:MANAGED -->';
const MANUAL_START = '<!-- OMK:AGENTS-INIT:MANUAL:START -->';
const MANUAL_END = '<!-- OMK:AGENTS-INIT:MANUAL:END -->';
...
export async function agentsInit(options: AgentsInitOptions = {}): Promise<void> {
  ...
  const backupRoot = join(cwd, '.omk', 'backups', 'agents-init', new Date().toISOString().replaceAll(':', '-'));
  ...
  console.log('oh-my-kiro AGENTS bootstrap');
  console.log('===========================\n');
  ...
  for (let index = 0; index < plannedDirs.length; index += 1) {
    ...
    const decision = await syncManagedAgentsFile(
      ...
      summary,
      backupRoot,
      rootOverlayRisk ? 'active omk session detected for project root AGENTS.md' : undefined,
    );
  ...
  console.log('- Preserves manual notes only for files already managed by omk agents-init.');
  console.log('- Never overwrites unmanaged AGENTS.md files unless you pass --force.');
  console.log('- Avoids rewriting project-root AGENTS.md while an active omk session is running.\n');
