...
describe('loadNotificationConfig', () => {
  ...
  it('returns parsed config when file exists', async () => {
    ...
    writeFileSync(join(omkDir, 'notifications.json'), JSON.stringify(configData));
  ...
  it('returns null for invalid JSON', async () => {
    ...
    const omkDir = join(tmpDir, '.omk');
    mkdirSync(omkDir, { recursive: true });
    writeFileSync(join(omkDir, 'notifications.json'), 'not-json');
